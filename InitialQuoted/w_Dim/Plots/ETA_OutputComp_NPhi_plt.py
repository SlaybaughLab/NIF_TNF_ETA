...
datalist = ['ETA_Output_Full_NPhi.csv','BareGodivaTgt_Full_NPhi.csv',
            'ETA_Output_Full_NPhi_LB.csv','ETA_Output_Full_NPhi_UB.csv']
label=['\\textbf{ETA Spectrum}','\\textbf{Objective TN+PFNS}',
       '\\textbf{1\\boldmath{$\sigma$} Error Range}','\\textbf{1$\sigma$ Error Range}']
...
s=10
linewidth=['2.0','2.0','0.25','0.25']
linestyle=[':', '-', '-', '-']
dashes=[[2, 3, 2, 3],[10, 0.1], [10, 0.1], [10, 0.1]]
...
# Shade the uncertainty bounds
ax1.fill_between(dataX[0], dataY[2], dataY[3], facecolor='grey', alpha=0.5,
                label='1 sigma range')

# Create Legend
rect = Rectangle((0, 0), 1, 1, fc="grey", alpha=0.5)
ax1.legend([line1,line2,rect], [label[0],label[1],label[2]],borderaxespad=0.75, 
            loc=2, fontsize=12, handlelength=5, borderpad=0.5, labelspacing=0.75, 
